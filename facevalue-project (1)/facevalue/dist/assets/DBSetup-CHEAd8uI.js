import{r as t,j as e}from"./index-BojVBYry.js";const y=`-- FACEVALUE DATABASE SCHEMA
...
create policy "admin read profiles" on user_profiles
  for select using (public.is_admin());`,f=`-- Add survey_id to celebrities (required by this app)
...
-- alter table celebrities alter column survey_id set not null;`,g=`-- Add profile_image to celebrities (used in leaderboard and result cards)
alter table celebrities
  add column if not exists profile_image text;`,x=`-- Backfill profile_image from the first uploaded photo for each celebrity
update celebrities c
set profile_image = (
...
where c.profile_image is null;`,v=`-- Enable realtime inserts for ratings so the Results board updates live
alter publication supabase_realtime add table public.ratings;`,S=`-- If surveys is missing status or is_active (schema cache errors)
...
-- alter table surveys drop column if exists celebrity2_id;`,j=`-- Run in Supabase SQL Editor (as project owner / postgres)
...
using (bucket_id = 'celebrity-photos' and auth.role() = 'authenticated');`,C=`-- Requires public.celebrities to exist. If this fails, run the full schema from "Full SQL Schema" above.
...
using (
  exists (select 1 from public.user_profiles where id = auth.uid() and role = 'admin')
);`,k=`-- Fix "new row violates row-level security policy" on public.celebrity_photos
...
);`,L=`-- Fix admin inserts on both public.celebrities and public.celebrity_photos
...
);`,E=`-- Add Rewards System (Gamification)
...
$$ language plpgsql security definer;`,X=[{n:1,title:"Create a Supabase Project",desc:"Go to supabase.com → New Project. Choose a region close to your users (e.g. Southeast Asia). Save your database password."},{n:2,title:"Run the SQL Schema",desc:"Go to SQL Editor → New Query. Copy the schema below, paste it in, and click Run.",hasSQL:!0},{n:3,title:"Configure Environment Variables",desc:"Copy your Project URL and anon key from Settings → API. Add them to your .env.local file.",code:`VITE_SUPABASE_URL=https://your-project.supabase.co
VITE_SUPABASE_ANON_KEY=your-anon-key`},{n:4,title:"Create Storage Bucket",desc:'In Supabase → Storage → New bucket. Name it exactly: celebrity-photos. Enable "Public bucket" toggle. If you use another name, set VITE_STORAGE_BUCKET in .env.local to match.',code:"VITE_STORAGE_BUCKET=celebrity-photos"},{n:5,title:"Enable Email Authentication",desc:"In Supabase → Authentication → Providers → Email → Enable. Optionally disable email confirmation for testing."},{n:6,title:"Create Admin User",desc:'Register at /login. Then in Table Editor → user_profiles → find your row → change the role column to "admin". You can now log in via the Admin tab.'}];function Z(){const[s,r]=t.useState(!1),[T,o]=t.useState(!1),[R,a]=t.useState(!1),[B,l]=t.useState(!1),[I,n]=t.useState(!1),[N,c]=t.useState(!1),[A,d]=t.useState(!1),[Q,u]=t.useState(!1),[z,p]=t.useState(!1),[P,m]=t.useState(!1),[F,b]=t.useState(!1),[M,h]=t.useState(!1);function _(){navigator.clipboard.writeText(y),r(!0),setTimeout(()=>r(!1),2e3)}function H(){navigator.clipboard.writeText(f),o(!0),setTimeout(()=>o(!1),2e3)}function U(){navigator.clipboard.writeText(g),a(!0),setTimeout(()=>a(!1),2e3)}function $(){navigator.clipboard.writeText(x),l(!0),setTimeout(()=>l(!1),2e3)}function G(){navigator.clipboard.writeText(v),n(!0),setTimeout(()=>n(!1),2e3)}function O(){navigator.clipboard.writeText(S),c(!0),setTimeout(()=>c(!1),2e3)}function q(){navigator.clipboard.writeText(w),d(!0),setTimeout(()=>d(!1),2e3)}function D(){navigator.clipboard.writeText(E),h(!0),setTimeout(()=>h(!1),2e3)}function Y(){navigator.clipboard.writeText(j),u(!0),setTimeout(()=>u(!1),2e3)}function V(){navigator.clipboard.writeText(C),p(!0),setTimeout(()=>p(!1),2e3)}function K(){navigator.clipboard.writeText(k),m(!0),setTimeout(()=>m(!1),2e3)}function W(){navigator.clipboard.writeText(L),b(!0),setTimeout(()=>b(!1),2e3)}return e.jsxs("div",{children:[e.jsx("div",{className:"admin-page-header",children:e.jsxs("div",{children:[e.jsx("h2",{className:"admin-page-title",children:"DB Setup"}),e.jsx("p",{className:"admin-page-subtitle",children:"Step-by-step Supabase configuration guide"})]})}),X.map(i=>e.jsxs("div",{className:"step-card",children:[e.jsx("div",{className:"step-number",children:i.n}),e.jsxs("div",{className:"step-content",style:{flex:1},children:[e.jsx("h4",{children:i.title}),e.jsx("p",{children:i.desc}),i.code&&e.jsx("div",{className:"code-block",style:{marginTop:"12px",fontSize:"0.75rem"},children:i.code}),i.hasSQL&&e.jsx("button",{className:"btn btn-outline btn-sm",onClick:_,style:{marginTop:"12px"},children:s?"✓ Copied!":"⎘ Copy Schema SQL"})]})]},i.n)),e.jsxs("div",{className:"card",style:{marginTop:"24px"},children:[e.jsxs("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"16px"},children:[e.jsx("h3",{style:{fontFamily:"'Syne', sans-serif",fontSize:"0.95rem"},children:"Full SQL Schema"}),e.jsx("button",{className:"btn btn-outline btn-sm",onClick:_,children:s?"✓ Copied!":"⎘ Copy"})]}),e.jsx("div",{className:"code-block",children:y})]}),e.jsxs("div",{className:"card",style:{marginTop:"24px"},children:[e.jsxs("h3",{style:{fontFamily:"'Syne', sans-serif",fontSize:"0.95rem",marginBottom:"8px"},children:["Existing database: missing ",e.jsx("code",{style:{fontSize:"0.85em"},children:"celebrities.survey_id"})]}),e.jsxs("p",{style:{fontSize:"0.875rem",color:"var(--muted)",marginBottom:"16px",lineHeight:1.6},children:["If the Celebrities page shows an error about ",e.jsx("code",{children:"survey_id"})," or the schema cache, run the script below in the Supabase SQL Editor, then wait a few seconds (or refresh the Table Editor) so the API picks up the new column."]}),e.jsx("button",{className:"btn btn-outline btn-sm",onClick:H,style:{marginBottom:"12px"},children:T?"✓ Copied!":"⎘ Copy migration SQL"}),e.jsx("div",{className:"code-block",children:f})]}),e.jsxs("div",{className:"card",style:{marginTop:"24px"},children:[e.jsxs("h3",{style:{fontFamily:"'Syne', sans-serif",fontSize:"0.95rem",marginBottom:"8px"},children:["Existing database: missing ",e.jsx("code",{style:{fontSize:"0.85em"},children:"celebrities.profile_image"})]}),e.jsx("p",{style:{fontSize:"0.875rem",color:"var(--muted)",marginBottom:"16px",lineHeight:1.6},children:"Run this once to store a profile image URL per celebrity. The Results leaderboard uses this value for the circular avatar beside each name."}),e.jsx("button",{className:"btn btn-outline btn-sm",onClick:U,style:{marginBottom:"12px"},children:R?"✓ Copied!":"⎘ Copy profile image migration SQL"}),e.jsx("div",{className:"code-block",children:g})]}),e.jsxs("div",{className:"card",style:{marginTop:"24px"},children:[e.jsx("h3",{style:{fontFamily:"'Syne', sans-serif",fontSize:"0.95rem",marginBottom:"8px"},children:"Optional backfill: use the first photo as the avatar"}),e.jsx("p",{style:{fontSize:"0.875rem",color:"var(--muted)",marginBottom:"16px",lineHeight:1.6},children:"If you already uploaded celebrity photos, run this after adding the column to populate avatars automatically."}),e.jsx("button",{className:"btn btn-outline btn-sm",onClick:$,style:{marginBottom:"12px"},children:B?"✓ Copied!":"⎘ Copy profile-image backfill SQL"}),e.jsx("div",{className:"code-block",children:x})]}),e.jsxs("div",{className:"card",style:{marginTop:"24px"},children:[e.jsxs("h3",{style:{fontFamily:"'Syne', sans-serif",fontSize:"0.95rem",marginBottom:"8px"},children:["Realtime: add ",e.jsx("code",{style:{fontSize:"0.85em"},children:"ratings"})," to ",e.jsx("code",{children:"supabase_realtime"})]}),e.jsx("p",{style:{fontSize:"0.875rem",color:"var(--muted)",marginBottom:"16px",lineHeight:1.6},children:"This is required for live leaderboard refreshes when new ratings are inserted. Run it once in the Supabase SQL Editor if realtime is not already enabled."}),e.jsx("button",{className:"btn btn-outline btn-sm",onClick:G,style:{marginBottom:"12px"},children:I?"✓ Copied!":"⎘ Copy realtime enable SQL"}),e.jsx("div",{className:"code-block",children:v})]}),e.jsxs("div",{className:"card",style:{marginTop:"24px"},children:[e.jsxs("h3",{style:{fontFamily:"'Syne', sans-serif",fontSize:"0.95rem",marginBottom:"8px"},children:["Existing database: missing ",e.jsx("code",{style:{fontSize:"0.85em"},children:"surveys.status"})," or ",e.jsx("code",{children:"surveys.is_active"})]}),e.jsx("p",{style:{fontSize:"0.875rem",color:"var(--muted)",marginBottom:"16px",lineHeight:1.6},children:"The app supports either column. Run the script below to add both and keep them in sync, then reload the app."}),e.jsx("button",{className:"btn btn-outline btn-sm",onClick:O,style:{marginBottom:"12px"},children:N?"✓ Copied!":"⎘ Copy surveys migration SQL"}),e.jsx("div",{className:"code-block",children:S})]}),e.jsxs("div",{className:"card",style:{marginTop:"24px"},children:[e.jsxs("h3",{style:{fontFamily:"'Syne', sans-serif",fontSize:"0.95rem",marginBottom:"8px"},children:["Legacy schema: ",e.jsx("code",{children:"celebrity1_id"})," / ",e.jsx("code",{children:"celebrity2_id"})," NOT NULL on ",e.jsx("code",{children:"surveys"})]}),e.jsxs("p",{style:{fontSize:"0.875rem",color:"var(--muted)",marginBottom:"16px",lineHeight:1.6},children:["If creating a survey fails with a null / not-null error on ",e.jsx("code",{children:"celebrity1_id"})," or ",e.jsx("code",{children:"celebrity2_id"}),", run the script below. This app attaches celebrities per survey via ",e.jsx("code",{children:"celebrities.survey_id"}),", not those two columns."]}),e.jsx("button",{className:"btn btn-outline btn-sm",onClick:q,style:{marginBottom:"12px"},children:A?"✓ Copied!":"⎘ Copy legacy celebrity columns fix"}),e.jsx("div",{className:"code-block",children:w})]}),e.jsxs("div",{className:"card",style:{marginTop:"24px"},children:[e.jsxs("h3",{style:{fontFamily:"'Syne', sans-serif",fontSize:"0.95rem",marginBottom:"8px"},children:["Storage: create ",e.jsx("code",{children:"celebrity-photos"})," bucket (fixes “Bucket not found”)"]}),e.jsxs("p",{style:{fontSize:"0.875rem",color:"var(--muted)",marginBottom:"16px",lineHeight:1.6},children:["Run the script below in the ",e.jsx("strong",{children:"SQL Editor"})," once. It registers the public bucket and policies so logged-in admins can upload and anyone can read images. Then refresh Photo Upload."]}),e.jsx("button",{className:"btn btn-outline btn-sm",onClick:Y,style:{marginBottom:"12px"},children:Q?"✓ Copied!":"⎘ Copy storage bucket SQL"}),e.jsx("div",{className:"code-block",children:j})]}),e.jsxs("div",{className:"card",style:{marginTop:"24px"},children:[e.jsxs("h3",{style:{fontFamily:"'Syne', sans-serif",fontSize:"0.95rem",marginBottom:"8px"},children:["Missing table: ",e.jsx("code",{children:"public.celebrity_photos"})]}),e.jsxs("p",{style:{fontSize:"0.875rem",color:"var(--muted)",marginBottom:"16px",lineHeight:1.6},children:["If Photo Upload or the API says the ",e.jsx("code",{children:"celebrity_photos"})," table is missing from the schema cache, run the script below. You need ",e.jsx("code",{children:"celebrities"})," (and usually ",e.jsx("code",{children:"user_profiles"})," for the admin policy) already present; otherwise run the ",e.jsx("strong",{children:"Full SQL Schema"})," first."]}),e.jsx("button",{className:"btn btn-outline btn-sm",onClick:V,style:{marginBottom:"12px"},children:z?"✓ Copied!":"⎘ Copy celebrity_photos table SQL"}),e.jsx("div",{className:"code-block",children:C})]}),e.jsxs("div",{className:"card",style:{marginTop:"24px"},children:[e.jsx("h3",{style:{fontFamily:"'Syne', sans-serif",fontSize:"0.95rem",marginBottom:"8px"},children:"Admin content fix for existing surveys"}),e.jsxs("p",{style:{fontSize:"0.875rem",color:"var(--muted)",marginBottom:"16px",lineHeight:1.6},children:["If adding a celebrity or its photos in an existing survey fails, run this block first. It replaces legacy policies and ensures both celebrity and photo inserts use ",e.jsx("code",{children:"WITH CHECK"})," for admins."]}),e.jsx("button",{className:"btn btn-outline btn-sm",onClick:W,style:{marginBottom:"12px"},children:F?"✓ Copied!":"⎘ Copy admin content RLS SQL"}),e.jsx("div",{className:"code-block",children:L})]}),e.jsxs("div",{className:"card",style:{marginTop:"24px"},children:[e.jsxs("h3",{style:{fontFamily:"'Syne', sans-serif",fontSize:"0.95rem",marginBottom:"8px"},children:["Fix RLS: ",e.jsx("code",{children:"new row violates row-level security policy"})," on ",e.jsx("code",{children:"celebrity_photos"})]}),e.jsx("p",{style:{fontSize:"0.875rem",color:"var(--muted)",marginBottom:"16px",lineHeight:1.6},children:"If photo upload fails with an RLS policy violation, run this SQL in Supabase SQL Editor. It allows authenticated users to read photos and only admin users to insert/update/delete photo rows."}),e.jsx("button",{className:"btn btn-outline btn-sm",onClick:K,style:{marginBottom:"12px"},children:P?"✓ Copied!":"⎘ Copy celebrity_photos RLS fix SQL"}),e.jsx("div",{className:"code-block",children:k})]}),e.jsxs("div",{className:"card",style:{marginTop:"24px"},children:[e.jsx("h3",{style:{fontFamily:"'Syne', sans-serif",fontSize:"0.95rem",marginBottom:"8px"},children:"Add Rewards System (Gamification)"}),e.jsx("p",{style:{fontSize:"0.875rem",color:"var(--muted)",marginBottom:"16px",lineHeight:1.6},children:"Enable gamification with points, achievements, and tier progression. Users earn 10 points per rating, unlock badges for milestones (10/50/100/500 ratings), and progress through tiers (Bronze → Silver → Gold → Platinum). Run this SQL to add the rewards system tables, RLS policies, and helper functions."}),e.jsx("button",{className:"btn btn-outline btn-sm",onClick:D,style:{marginBottom:"12px"},children:M?"✓ Copied!":"⎘ Copy rewards system SQL"}),e.jsx("div",{className:"code-block",children:E})]}),"    "]})}export{Z as default};
